...
/// usually the only device in the module. On constructing a Mapping adapter,
...
class Mapping {
 public:
  ...
  /// Looks up a Resource @p maps_to refers to, if any.
  ///
  ...
  [[nodiscard]] auto lookup(ResourceSpec maps_to) const -> ResourceType {
    if (const auto kind = dyn_cast<Attribute>(maps_to); kind) {
  ...
  /// @copydoc lookup(ResourceSpec)
  template <class ResourceType = Resource>
  [[nodiscard]] auto lookup(MapsToAttr maps_to) const -> ResourceType {
    if (auto id_ref = dyn_cast<FlatSymbolRefAttr>(maps_to); id_ref) {
      return by_id_.lookup<ResourceType>(id_ref.getAttr());
    }
    return by_kind_.getInstance<ResourceType>(maps_to);
  }
  ...
  /// @tparam ResourceType  Expected resource type.
  ///
  /// @retval ResourceType  Resource @p mappable is mapped to.
  /// @retval nullptr       No resource for @p mappable or of different type.
  template <class ResourceType = Resource>
  [[nodiscard]] auto lookup(Operation* mappable) const -> ResourceType {
    if (auto [op, attr] = get(mappable); attr) {
      return lookup<ResourceType>(attr);
    }
    return nullptr;
  }
  ...
  /// Looks up the Resource @p mappable is mapped to, setting it to @p fallback
  /// if it is unmapped.
  ///
  /// @tparam ResourceType  Expected resource type.
  ///
  /// @retval ResourceType  Resource @p mappable is now mapped to.
  /// @retval nullptr       No resource for @p fallback or of different type.
  template <class ResourceType = Resource>
  auto getOrMap(Operation* mappable, ResourceSpec fallback) -> ResourceType {
    if (auto resource = this->lookup(mappable); resource) {
      if constexpr (std::is_same_v<ResourceType, Resource>) {
        return resource;
      } else {
    ...
    map(mappable, fallback);
    return lookup<ResourceType>(fallback);
  }
